...
#Remember to switch backslashes to forward slashes
```

# Installing and loading your libraries 
(aka packages to run specific functions)

Below are three packages you will need for this exercise. Once you install them, you need to use the function 'library' to call in the package. You only ever need to install a package once, but you always need to call in a package every time you open R.
...
Functions will vary depending on the source file and type! 
```{r}
#rm(list=ls())
...
#pqcc_raw_vars <- data.frame("label" = sapply(pqcc_raw, 
                         # function(x) attr(x, "label")) %>% #as.character(),  "labelled" = sapply(pqcc_raw, is.labelled))
#pqcc_raw_vars
```
...
#LABEL AGERAW
pqcc_raw$age_raw <-factor(pqcc_raw$age_raw, levels=c(1,2,3),  labels=c("15-19", "20-29", "30-48"), ordered=F)#changed to F for ordered
...
pqcc_raw$marital_stat3 <-factor(pqcc_raw$marital_stat3, levels=c(1,3),  labels=c("Not married", "Married"), ordered=F)#not married here is equal to Single/Widowed/Separated/Divorced/Cohabiting
...
```{r}
#num_births
# Here I am clusturing those with 4 or more births into one level so that level 5(5 or more births) is equql to 4
...
## Here we will generate a new variable pcc_explaim meds_1 which will have 3 factors instead of 4 factors. All the 4th option  factors which translate to (Did not get any medicine) were moved into the 3rd category of yes all the time due to conservative coding.
```{r}
#Method 1
...
**Here we are creating a binary exposure variable using a previous variable pcc_support_labor. Our binary level will combine levels 0 (No never) and 1( Yes, a few times) as the level 0=No then the 3 other levels(2,3,4- yes most of the time, yes all of the time and I did not want someone to stay with me as our level 1 =Yes). Those that responded that I did not want someone to stay with me are part of the 3rd level(Yes all of the time). This is once again because of conservative coding and here we are making the assumption that had the person wanted someone to accompany them they they would have been allowed to have someone with them.**
```{r}
#EXPOSURE Variable:
...
Here I will apply the exclusion critearia to determine the final sample size I will be working with. I will be creating a new data.frame for each time I do a subset so I can track my numbers
```{r results='hide'}
