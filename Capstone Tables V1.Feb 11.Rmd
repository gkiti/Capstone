...
This tutorial will provide useful tips and tricks for descriptive analyses and cross-tabulatons. These tools will be most helpful in building your Tables 1 and 2 for your research project. In addition, we have also provided some statistical tests to further explore relationships between categorical/continuos variables. Brief descriptions for these statistical tests are provided below.

```{r}
setwd("~/Dropbox/Gift captstone/Data")
```

## Call in the dataset you created last week (from raw.dta)
```{r}
#read in the dataset created from the previous week - the name may be different
pqcc_raw_3 <- read.csv("final_data.csv")

#pqcc_raw_3 <- data.frame(pqcc_raw_3)

#notice that the CSV file retains labels created in R, but it does not retain STATA labels
```
...
Using T-test: We use t-test when we are dealing with 1 categorical variable(2 levels) and 1 continuous varible
```{r results='hide'}
#t-TEST
...
In computing the p value for table 3 we shar either carry out an anova test or a t test. We will carry out an anova test if we are dealing with 1 variable that is continuous and another one that is categorical in 2 levels. T-test is used when we are deadling with 1 continuous variable and a categorical variable that only has 2 levels.
...
# Exploring specific variables --  Part 3

```{r eval=FALSE}
###YEARS IN THE USA (CATEGORICAL)

#LOOK AT DATA
hist(df_raw$ayrsusa)
summary(df_raw$ayrsusa)

table(df_raw$ayrsusa, exclude=NULL) #include NAs
freq(df_raw$ayrsusa, cumul = FALSE)

#LOGIC CHECK - Did anyone live in US more years than they are old?
#METHOD 1
age_check <- subset(df_raw, df_raw$ayrsusa>df_raw$age)

##METHOD 2
check <- df_raw$ayrsusa>df_raw$age
sum(check, na.rm=TRUE) #should equal to 0

###METHOD 3
which(df_raw$ayrsusa>df_raw$ayrsusa)

#WHY ARE DATA MISSING? 
#If they were born in the US (N=63 women), they were not asked this question.

ctable(df_raw$ayrsusa, df_raw$momborn, prop = "n")
#prop = "n"
#prop = "t"
#prop = "c"
#prop = "r"

ctable(df_raw$ayrsusa, df_raw$entire_life, prop = "n")
```
